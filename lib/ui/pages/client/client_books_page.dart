...
class ClientBooksPage extends StatefulWidget {
  const ClientBooksPage({super.key});

  @override
  State<ClientBooksPage> createState() => _ClientBooksPageState();
}

class _ClientBooksPageState extends State<ClientBooksPage> {
  ...
  Future<void> _fetchLivres({int? targetPage}) async {
    setState(() {
      _isLoading = true;
      if (targetPage != null) _page = targetPage;
    });
    try {
  ...
  Future<void> _openDetailsPage(Livre livre) async {
    final changed = await Navigator.of(context).pushNamed(
      AppRouter.clientBookDetailPage,
      arguments: livre.id,
    );
    if (changed == true) await _fetchLivres();
  }
  ...
  Widget _buildCategorieSelect() {
    ...
    return DropdownButtonFormField<String>(
      ...
      items: [
        ...
        ..._categories.map((item) => DropdownMenuItem<String>(value: item.id, child: Text(item.nom))),
      ],
      ...
      child: Row(
        children: [
          ...
          AppButton.secondary(
            ...
            onPressed: _page > 1 ? () => _fetchLivres(targetPage: _page - 1) : null,
          ),
          const SizedBox(width: 8),
          AppButton.primary(
            label: 'Next',
            icon: Icons.chevron_right_rounded,
            onPressed: _page < _pages ? () => _fetchLivres(targetPage: _page + 1) : null,
          ),
        ],
      ),
    );
